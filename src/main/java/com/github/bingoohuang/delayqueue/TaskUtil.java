...
public class TaskUtil {
  @SneakyThrows
  public static <T> Pair<T, Boolean> timeoutRun(
      ExecutorService executorService, Callable<? extends T> runnable, int timeout) {
  ...
  public static DateTime emptyThenNow(DateTime dateTime, CronExpression cron) {
    return dateTime != null
        ? dateTime
        : cron == null ? DateTime.now() : cron.nextTimeAfter(DateTime.now());
  }

  /**
   * 获取Spring Bean的默认名字。参考org.springframework.context.annotation.AnnotationBeanNameGenerator文档。
   * Derive a default bean name from the given bean definition. The default implementation simply
   * builds a decapitalized version of the short class name: e.g. "mypackage.MyJdbcDao" -
   * "myJdbcDao".
   *
