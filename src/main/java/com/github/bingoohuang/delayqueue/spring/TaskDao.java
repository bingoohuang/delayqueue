...
public interface TaskDao {
    ...
    @Sql("insert into $$(TASK_ID, RELATIVE_ID, CLASSIFIER, TASK_NAME, TASK_SERVICE, STATE, RUN_AT, " +
            ...
            " #t.timeout#, #t.startTime#, #t.endTime#, #t.result#, #t.attachment#, #t.var1#, #t.var2#, #t.var3#, #t.createTime#)  /* end */ ")
    int add(List<TaskItem> tasks, @Dynamic String taskTableName);

    @Sql("select TASK_ID, RELATIVE_ID, CLASSIFIER, TASK_NAME, TASK_SERVICE, STATE, RUN_AT, TIMEOUT, START_TIME, END_TIME, RESULT, ATTACHMENT, VAR1,VAR2,VAR3, CREATE_TIME " +
            "from $$ where TASK_ID = ## ")
    ...
    @Sql("update $$ set STATE = #_1.state#, END_TIME = #_1.endTime#, RESULT = #_1.result# where TASK_ID = #_1.taskId# and STATE = #_2#")
    int end(TaskItem task, String fromState, @Dynamic String taskTableName);

    @Sql("update $$ set STATE = #_4#, END_TIME = NOW(), RESULT = #_1# where TASK_ID in (/* in _2 */) and STATE = #_3#")
